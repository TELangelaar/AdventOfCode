...
pub mod parsing {
    ...
    pub fn lines_to_vec(content: &str) -> anyhow::Result<Vec<u32>> {
        ...
        for line in content.lines() {
            results.push(
                line.trim()
                    .parse::<u32>()
                    .with_context(|| format!("Could not parse '{}' into u32", line))?,
            ); // We can just use this ? operator here because it will cascade the error back to the caller
        }
    ...
    mod tests {
        ...
        fn one_result() {
            ...
            assert_eq!(vec![123, 456, 789], lines_to_vec(&content).unwrap());
        }
    }
}
